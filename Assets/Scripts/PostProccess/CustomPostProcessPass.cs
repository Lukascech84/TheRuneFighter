...
public class CustomPostProcessPass : ScriptableRenderPass
{
    ...
    public CustomPostProcessPass(Material bloomMaterial, Material compositeMaterial)
    {
        ...
        if (SystemInfo.IsFormatSupported(GraphicsFormat.R8G8B8A8_UNorm, usage))
        {
            hdrFormat = GraphicsFormat.R8G8B8A8_UNorm;
        }
        else
        {
    ...
    private void SetupBloom(CommandBuffer cmd, RTHandle source)
    {
        ...
        {
            Debug.LogError("Bloom MipDown[0] is null! Check RTHandle allocation.");
            return;
        }
