...
public class CustomPostProcessRenderFeature : ScriptableRendererFeature
{
    ...
    public override void SetupRenderPasses(ScriptableRenderer renderer, in RenderingData renderingData)
    {
        if (renderingData.cameraData.cameraType == CameraType.Game)
        {
            ...
            if (colorTarget == null)
            {
                Debug.LogError("SetupRenderPasses: Color target is null!");
            }
            if (depthTarget == null)
            {
                Debug.LogError("SetupRenderPasses: Depth target is null!");
            }

            m_customPass.ConfigureTarget(colorTarget, depthTarget);
            m_customPass.ConfigureInput(ScriptableRenderPassInput.Color | ScriptableRenderPassInput.Depth);
        }
    }
